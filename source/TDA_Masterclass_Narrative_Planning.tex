...
\section*{Topic #1: #2}
\addcontentsline{toc}{section}{Topic #1: #2}
}
...
This document turns the topic-by-topic planning table into a more narrative
planning document. It is not yet a lecture script. Its purpose is to record the
story each topic needs to tell, the mathematical objects that must be introduced,
...
\planningbox{Story}{
The first topic should make the case for topology as a lens without pretending
...
\planningbox{Story}{
Topic 2 moves from qualitative shape to a combinatorial object on which topology
...
\planningbox{Story}{
Topic 3 introduces the filtration. The key move is that one complex is not
...
\planningbox{Story}{
Topic 4 closes the first-half definition. Homology is now read across the
...
\planningbox{Examples}{
Build the topic around two small calculations before showing a noisy point
cloud.
...
\section*{Pause and take stock}
\addcontentsline{toc}{section}{Pause and take stock}

\planningbox{Story}{
Topics 1--4 assemble persistent homology one construction at a time. Before
moving into larger applications, pause and make the complete modelling pipeline
explicit:
\[
...
interpreted: a feature of the scientific system, a feature of the observed
data, a class in $H_p(K_\varepsilon)$, an interval in a persistence module, a
point in a diagram, or a scalar summary of many diagram points. Claims become
weaker as more of the pipeline is forgotten.}

By this point, participants have already encountered most of these choices, but
some may have appeared to be inevitable parts of the mathematics. This pause
exposes them as modelling decisions and asks what each one permits the eventual
result to mean. Persistent homology is not one method applied after the data
have been prepared. It is a
...
Return to examples already used in Topics 1--4 and deliberately alter one choice
at a time:
\begin{itemize}
...
\planningbox{Story}{
Topic 5 is the first practical topic. The aim is not just to compute persistence,
...
\planningbox{DP relevance}{
This topic asks whether topology adds explanatory or predictive value beyond
...
\planningbox{Story}{
Topic 7 introduces the honest dynamic baseline. Given a sequence of time-indexed
...
\planningbox{Story}{
Topic 8 should be organised by problem type rather than by a list of advanced
